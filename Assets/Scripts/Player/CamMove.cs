...
{
    //private Vector3 originPos; // 카메라 원래 위치 변수
    //private Vector2 distance; // 인벤 열었을 때의 카메라 이동거리 변수
    //private void Start()
    //{
    //    originPos = new Vector3(0, 0, -10); // 카메라 원래 위치 초기화
    ...
    void Start()
    {
        originLocalPos = transform.localPosition;  // 자식 기준 원래 위치 저장
        distance = new Vector2(5, 0);              // 너무 크니까 5 정도로 줄이기
    }

    public void MoveRightCamera()
    {
        transform.localPosition = originLocalPos + (Vector3)distance;
    }
